    ...
    public ResponseEntity<?> signUp(
        ...
        try {
            ...
            if (profileImage != null) {
                log.info("fileName - {}", profileImage.getOriginalFilename());
            ...
            log.warn("파일 업로드 경로가 잘못되었거나 파일 저장에 실패하였습니다.");
...
//    @PreAuthorize("hasRole('ROLE_COMMON') or hasRole('ROLE_ADMIN')")
    ...
    public ResponseEntity<?> loadProfile(
        ...
        try {
            ...
            // -> 파일명은 데이터베이스를 조회해야 함
            profilePath = userService.getProfilePath(userInfo.getEmail());
            ...
            // 서버에 저정된 파일을 직렬화하여 바이트배열로 만들어서 가져옴
